...
- **summarizer** — when reading a file for context only (to understand what it does, not to edit or quote it). Returns filename + compressed summary; never the raw content. Re-read the file directly when you need to make an edit or quote specific text.
